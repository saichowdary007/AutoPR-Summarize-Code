...
def _determine_pr_type(title: str, description: str, commits: List[Dict[str, Any]]) -> str:
    ...
    elif any(kw in title_lower for kw in ["refactor", "clean", "simplify", "restructure"]):
        return "refactoring"
...
def _extract_key_changes(description: str, commits: List[Dict[str, Any]], files: List[Dict[str, Any]]) -> List[str]:
    ...
    if not changes:
        ...
        if total_additions > total_deletions:
            changes.append(f"Added {total_additions} lines of code across {len(files)} files")
        elif total_deletions > total_additions:
            changes.append(f"Removed {total_deletions} lines of code across {len(files)} files")
        else:
            changes.append(f"Modified {len(files)} files with balanced additions and deletions")
...
def _extract_testing_info(description: str, files: List[Dict[str, Any]]) -> Optional[str]:
        ...
        if any(dep_file in file["filename"] for dep_file in dependency_files):
            if file["patch"]:
                ...
                for line in added_lines:
                    ...
                    if npm_match:
                        dep_changes.append(f"Added: {npm_match.group(1)}@{npm_match.group(2)}")
                        continue
                    
                    # requirements.txt pattern
                    req_match = re.search(r'([a-zA-Z0-9_-]+)[=~<>]+([0-9.]+)', line)
                    if req_match:
                        dep_changes.append(f"Added: {req_match.group(1)}=={req_match.group(2)}")
                        continue
                    
                    # go.mod pattern
                    go_match = re.search(r'([a-zA-Z0-9_\-./]+)\s+v([0-9.]+)', line)
                    if go_match:
                        dep_changes.append(f"Added: {go_match.group(1)}@v{go_match.group(2)}")
                        continue
...
def _identify_potential_risks(files: List[Dict[str, Any]], description: str) -> List[str]:
