...
class GitHubService:
    ...
    async def post_review_comments(self, pr_number: int, review_results: Dict[str, Any]) -> None:
        """
        ...
        for category, issues in review_results.items():
            if isinstance(issues, list):  # Skip statistics or other non-list items
                for issue in issues:
                    ...
                    if not all(k in issue for k in ["file", "line", "severity", "issue", "recommendation"]):
                        continue
        ...
        for issue in all_issues:
            ...
            if issue.get('reference'):
                comment += f"**Reference:** [{issue['reference']}]({issue['reference']})"
